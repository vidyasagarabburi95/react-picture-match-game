...
class App extends Component {
  ...
  updateActiveTab = tabId => {
    this.setState({activeTabId: tabId, imgUrl: imagesList[0].imageUrl})
  }
  ...
  render() {
    const {timer, activeTabId, score, imgUrl, isTrue} = this.state
          ...
          <div>
            ...
            <div className="bg-con">
              ...
              <ul className="tabs-list">
                {tabsList.map(tab => (
                  <li key={tab.tabId} className="tab-buttons">
                    <button
                      type="button"
                      onClick={() => this.updateActiveTab(tab.tabId)}
                      className={activeTabId === tab.tabId ? 'active-tab' : ''}
                    >
                      {tab.displayText}
                    </button>
                  </li>
              ...
              <ul className="thumb-nails">
                {filteredImages.map(image => (
                  <li key={image.id} className="items">
                    <button
                      type="button"
                      onClick={() => this.onClickingImage(image.thumbnailUrl)}
                    >
                      <img
